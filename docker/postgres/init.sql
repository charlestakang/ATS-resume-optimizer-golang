...
-- Create GIN indexes for JSONB fields (for faster queries on audit data)
CREATE INDEX IF NOT EXISTS idx_audit_log_old_data_gin ON audit.audit_log USING GIN (old_data);
CREATE INDEX IF NOT EXISTS idx_audit_log_new_data_gin ON audit.audit_log USING GIN (new_data);
...
-- Create trigger for updated_at on analytics tables
CREATE TRIGGER trg_daily_stats_updated_at
BEFORE UPDATE ON analytics.daily_stats
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Note: Performance optimization settings should be configured in postgresql.conf
-- or via Docker environment variables. ALTER SYSTEM requires superuser privileges.
-- Add these to your Docker Compose environment or postgresql.conf:
...
DO $$ BEGIN
    CREATE TYPE resume_status AS ENUM ('draft', 'processing', 'completed', 'error');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
    CREATE TYPE optimization_status AS ENUM ('pending', 'processing', 'completed', 'failed');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- Create notification function for real-time updates (safe for tables without id)
...
VALUES ('database', 'INITIALIZE', '{"message": "Database initialized successfully"}', 'system', NOW());

-- Print success message
DO $$
BEGIN
    RAISE NOTICE 'ResumeSync Pro database initialization completed successfully';
    RAISE NOTICE 'Extensions enabled: uuid-ossp, pg_trgm, pg_stat_statements, btree_gin';
    RAISE NOTICE 'Schemas created: audit, analytics';
    RAISE NOTICE 'Custom types created: user_role, subscription_plan, subscription_status, resume_status, optimization_status';
    RAISE NOTICE 'Helper functions created: audit_trigger_func, update_updated_at_column, generate_slug, search_rank';
    RAISE NOTICE 'Indexes created: Standard B-tree and GIN indexes for performance';
    RAISE NOTICE 'Triggers created: updated_at trigger for analytics.daily_stats';
END $$;